...
it('can add the star name and star symbol properly', async() => {
    ...
    let name = "Horus";
    let symbol = "HRS";
    let instance = await StarNotary.deployed(name,symbol);
    let user = accounts[1];
    let starId = 5;
    await instance.createStar(name,starId,{from: user});
...
it('lets 2 users exchange stars', async() => {
    ...
    let user1 = accounts[1];
    let starId1 = 101;
    await instance.createStar('Rahmat', starId1, {from: user1});
    let user2 = accounts[2];
    let starId2 = 202;
    await instance.createStar('Muinat', starId2, {from: user2});
    // 2. Call the exchangeStars functions implemented in the Smart Contract
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let user1 = accounts[1];
    let starId = 213;
    await instance.createStar('Daud', starId, {from: user1});
    // 2. Call your method lookUptokenIdToStarInfo
    let starName = await instance.lookUptokenIdToStarInfo(starId);
    // 3. Verify if you Star name is the same
    assert.equal(starName, "Daud");
});